...
def extract_new_content(old_text: str, new_text: str, mode: str) -> str:
    ...
    added = [line for line in added if len(line) > 25]
...
def format_message(label: str, url: str, summary: dict, is_first_check: bool) -> str:
    note = (
        "\n<i>(first check for this source — establishing baseline, treat as informational)</i>"
        if is_first_check
        else ""
    )
    return (
...
def main():
    ...
    for source in sources:
        ...
        try:
            ...
            log.error(
                "Could not read snapshot for %s from storage: %s — skipping this run.",
                label,
                e,
            )
            any_failures = True
        ...
        if prior is None:
            ...
            storage.set_snapshot(
                sid, {"hash": new_hash, "text": new_text, "last_checked": run_time}
            )
            continue
        ...
        if summary is None:
            log.warning(
                "Could not summarize change for %s — skipping message, will retry next run if still new.",
                label,
            )
            any_failures = True
    ...
    if any_failures:
        sys.exit(2)  # non-zero exit so the GH Action run is visibly flagged
